...
namespace ItemSearchPlugin.Filters
{
    class SoldByNPCSearchFilter : SearchFilter
    {
        ...
        private readonly Dictionary<uint, HashSet<uint>> soldForCurrency = new();

        private enum SpecialCurrency : uint
        {
            [Description("大国防联军")] GrandCompany = uint.MaxValue - 10,
        }

        private class CurrencyOption
        {
            public bool Invert;
            public HashSet<uint> ItemHashSet = new();
            public string Name = string.Empty;
            public List<CurrencyOption?> SubOptions = new();
        ...
        public SoldByNPCSearchFilter(ItemSearchPluginConfig config, IDataManager dataManager) : base(config)
        {
            ...
            notSoldByNpcOption = new CurrencyOption
                { Invert = true, Name = "不可从NPC购买", ItemHashSet = soldForAnything, HideIfEmpty = false };
            soldByAnyNpcOption = new CurrencyOption
                { Name = "任意货币", ItemHashSet = soldForAnything, HideIfEmpty = false };
            ...
            soldForCurrency.Add((uint)SpecialCurrency.GrandCompany, new HashSet<uint>()); 
            ...
            Task.Run(() =>
            {
                try
                {
                    foreach (var gilShopItem in dataManager.GetSubrowExcelSheet<GilShopItem>())
                    {
                        ...
                        {
                            soldForAnything.Add(subrow.Item.Value.RowId);
                            soldForCurrency[1].Add(subrow.Item.Value.RowId);
                        }
                    }
                    ...
                    foreach (var gcScripShopItem in data.GetSubrowExcelSheet<GCScripShopItem>())
                    {
                        ...
                        {
                            soldForAnything.Add(subrow.Item.Value.RowId);
                            if (!soldForCurrency[(uint)SpecialCurrency.GrandCompany].Contains(subrow.Item.Value.RowId))
                                soldForCurrency[(uint)SpecialCurrency.GrandCompany].Add(subrow.Item.Value.RowId);

                            var gcScripShopCategory = data.Excel.GetSheet<GCScripShopCategory>().GetRow(subrow.RowId);
                            if (gcScripShopCategory.RowId == 0) continue;
                            var grandCompanyID = gcScripShopCategory.GrandCompany.RowId;
                            if (grandCompanyID < 1 || grandCompanyID > 3) continue;
                            if (!soldForCurrency[19 + grandCompanyID].Contains(subrow.Item.Value.RowId))
                                soldForCurrency[19 + grandCompanyID].Add(subrow.Item.Value.RowId);
                        }
                    }
        ...
        private CurrencyOption? GetCurrencyOption(uint itemId, string? tag = null, string? forceName = null)
        {
            try
            {
                ...
                return new CurrencyOption()
                {
                    Name = forceName ?? item.Name.ExtractText(),
                    ItemHashSet = soldForCurrency[itemId],
                    Tag = tag?.ToLower()
                };
            }
        ...
        public override string Name => "可从NPC购买";
        ...
        public override void DrawEditor()
        {
            ...
            ImGui.BeginChild($"###{NameLocalizationKey}Child", new Vector2(-1, 23 * ImGui.GetIO().FontGlobalScale),
                false, usingTags ? ImGuiWindowFlags.NoInputs : ImGuiWindowFlags.None);

            if (selectedCurrencyOption is not null && selectedCurrencyOption.SubOptions.Count > 0)
            {
            ...
            if (ImGui.BeginCombo("###SoldbyNPCSearchFilter_selection", selectedCurrencyOption?.Name ?? "未选择"))
            {
                foreach (var option in availableOptions)
                {
                    if (ImGui.Selectable(option?.Name ?? "未选择", selectedCurrencyOption == option))
                    {
            ...
            if (selectedCurrencyOption is not null && selectedCurrencyOption.SubOptions.Count > 0)
            {
                ImGui.SameLine();
                ImGui.SetNextItemWidth(-1);
                if (ImGui.BeginCombo("###SoldbyNPCSearchFilter_subselection", selectedSubOption?.Name ?? "任意"))
                {
                    foreach (var option in selectedCurrencyOption.SubOptions)
                    {
                        if (ImGui.Selectable(option?.Name ?? "任意", selectedCurrencyOption == option))
                        {
