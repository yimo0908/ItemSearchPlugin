...
namespace ItemSearchPlugin
{
    public static class ClassExtensions
    {
        ...
        public static bool AllowsRaceSex(this EquipRaceCategory erc, uint raceId, CharacterSex sex)
        {
            return sex switch
            {
                CharacterSex.Both when (!erc.Male || !erc.Female) => false,
                CharacterSex.Either when (erc is { Male: false, Female: false }) => false,
                CharacterSex.Female when (!erc.Female) => false,
                CharacterSex.Male when (!erc.Male) => false,
                _ => raceId switch
                {
                    0 => false,
                    1 => erc.Hyur,
        ...
        public static string DescriptionAttr<T>(this T source)
        {
            ...
            if (fi.GetCustomAttribute<DescriptionAttribute>() is { } attribute)
        ...
        {
            return float.TryParse(patch, out float number)
                ? number switch
                {
                    >= 1 and < 2 => "1.0",
                    >= 2 and < 3 => "ARR",
